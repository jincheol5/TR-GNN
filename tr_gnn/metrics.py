...
class Metrics:
    ...
    @staticmethod
    def compute_TR_macroF1(logit_list:list,label_list:list,threshold:float=0.5,eps:float=1e-8):
        # 1) concat
        logit_all=torch.cat(logit_list,dim=0).view(-1) # [all_B]
        label_all=torch.cat(label_list,dim=0).view(-1) # [all_B]

        # 2) sigmoid → hard prediction
        prob=torch.sigmoid(logit_all)
        pred=(prob>=threshold).float()

        # 3) confusion matrix
        tp=((pred==1) & (label_all==1)).sum().float()
        tn=((pred==0) & (label_all==0)).sum().float()
        fp=((pred==1) & (label_all==0)).sum().float()
        fn=((pred==0) & (label_all==1)).sum().float()

        # 4) class-wise F1
        # Positive class (reachable)
        if (tp+fn)==0:
            f1_pos=0.0
        else:
            f1_pos=(2*tp)/(2*tp+fp+fn+eps)

        # Negative class (non-reachable)
        if (tn+fp)==0:
            f1_neg=0.0
        else:
            f1_neg=(2*tn)/(2*tn+fp+fn+eps)
    ...
    def compute_TR_AUROC(logit_list:list,label_list:list):
        ...
        label_all=torch.cat(label_list,dim=0).view(-1) # [all_B,]
        prob=torch.sigmoid(logit_all)
        auroc=binary_auroc(prob,label_all).item()
        if math.isnan(auroc):
    ...
    def compute_TR_PRAUC(logit_list:list,label_list:list):
        ...
        label_all=torch.cat(label_list,dim=0).view(-1) # [all_B,]
        prob=torch.sigmoid(logit_all)
        prauc=binary_auprc(prob,label_all).item()
        if math.isnan(prauc):

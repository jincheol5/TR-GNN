...
class TGN(nn.Module):
    ...
    def forward(self,data_loader,device,mode=None):
        ...
        for batch in data_loader:
            ...
            memory=self.memory_updater(traj=batch['init_traj'],memory=memory,source=batch['src'],target=batch['tar'],delta_t_vec=embedded_mem_t) # [N,latent_dim]
            ...
            match self.emb:
                case 'time':
                    z=self.embedding(memory=memory,delta_t=batch['emb_t'],tar_idx=batch['tar']) # [B,latent_dim]
                case 'sum'|'attn':
                    z=self.embedding(traj=batch['init_traj'],x=x,delta_t_vec=embedded_emb_t,neighbor_mask=batch['n_mask'],tar_idx=batch['tar'],memory=memory) # [B,latent_dim]
            logit=self.linear(z)
            logit_list.append(logit)
        return logit_list
...
class TR_GNN(nn.Module):
    ...
    def forward(self,data_loader,device,mode:Literal['train','test']='train'):
        ...
        for batch in data_loader:
            ...
            """
            embedded_mem_t=self.time_encoder(batch['mem_t']) # [B,N,latent_dim]
            memory=self.memory_updater(traj=pre_traj,memory=memory,source=batch['src'],target=batch['tar'],delta_t_vec=embedded_mem_t) # [N,latent_dim]

            """
            2. embedding
            """
            embedded_emb_t=self.time_encoder(batch['emb_t']) # [B,N,latent_dim]
            z=self.embedding(traj=pre_traj,x=x,delta_t_vec=embedded_emb_t,neighbor_mask=batch['n_mask'],tar_idx=batch['tar'],memory=memory) # [B,latent_dim]
            logit=self.linear(z)
            logit_list.append(logit)

...
class GraphUtils:
    ...
    @staticmethod
    def compute_tR_step(num_nodes:int,source_id:int,edge_event:tuple=None,init:bool=False,gamma:torch.Tensor=None,time_table:torch.Tensor=None):
        """
        edge_event: tuple, (src,tar,ts)
        gamma: [N,2] tensor, (tR,visited time)
        time_table: [N,N] tensor, (tar_id,src_id(index) and interaction time(value))
        """
        if init:
            gamma=torch.zeros((num_nodes,3),dtype=torch.float) # tR,time,ts
            gamma[:,0]=0.0 # tR
            ...
            time_table=torch.zeros((num_nodes,num_nodes),dtype=torch.float)
        else:
            src,tar,ts=edge_event
            if gamma[src,0].item()==1.0 and gamma[tar,0].item()==0.0 and gamma[src,1].item()<ts:
                gamma[tar,0]=1.0
                gamma[tar,1]=ts
            time_table[tar,src]=ts 
        return gamma,time_table
    ...
    def convert_event_stream_to_dataset(event_stream:list,num_nodes:int,source_id:int):
        ...
        for i,edge_event in enumerate(event_stream):
            ...
            src,tar,ts=edge_event
            r_list.append(gamma[:,:1])
            interacted_t=time_table[tar].unsqueeze(-1) # [N,1]
            t_list.append(torch.abs(interacted_t-ts))
            src_list.append(src)
            tar_list.append(tar)
            label_list.append(gamma[tar,0].item())
